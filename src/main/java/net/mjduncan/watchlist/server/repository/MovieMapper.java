...
public interface MovieMapper {
    ...
    @Select("SELECT movies.imdb_id, movies.title, movies.year, movies.imdb_rating, movies.tomatoes_rating FROM accounts_movies INNER JOIN movies ON movie_id = movies.imdb_id WHERE account_id = #{userId}")
    @Results({
            @Result(property = "imdbID", column = "imdb_id", id = true),
            @Result(property = "title", column = "title"),
            @Result(property = "year", column = "year"),
    ...
    @Select("SELECT movies.imdb_id, movies.title, movies.year, movies.imdb_rating, movies.tomatoes_rating FROM accounts_movies INNER JOIN movies ON movie_id = movies.imdb_id WHERE account_id = #{userId} AND imdb_id = #{movieId}")
    @Results({
            @Result(property = "imdbID", column = "imdb_id", id = true),
            @Result(property = "title", column = "title"),
            @Result(property = "year", column = "year"),
    ...
    @Insert("INSERT INTO movies (imdb_id, title, year, imdb_rating, tomatoes_rating) VALUES (#{imdbID}, #{title}, #{year}, #{imdbRating}, #{tomatoesRating})")
